# Quick memory comparison: favara_imbs scaled 10x (instead of 50x), 10 reps
# instead of 30. Demonstrates the leak fix in a few minutes instead of an hour.
...
data(favara_imbs)
fi_big <- rbindlist(lapply(seq_len(10), function(i) {
...
run_one <- function(method_label, method_value) {
    ...
    res <- did_multiplegt_dyn(
        ...
        graph_off = TRUE, bootstrap = list(10, 1)
    )
    el <- as.numeric(Sys.time() - t0, units = "secs")

    cat(sprintf("[A] elapsed:                    %5.1f s\n", el))
    cat(sprintf("[A] RSS after bootstrap=10:     %5.2f GB\n", get_rss_gb()))
