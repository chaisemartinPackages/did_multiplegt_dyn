clear all
cd "/Users/anzony.quisperojas/Library/CloudStorage/Dropbox/data_eco/data_dyn"
do "/Users/anzony.quisperojas/Documents/GitHub/DCDH_TODO/code/new_core_latest.do"

cd "/Users/anzony.quisperojas/Documents/GitHub/did_multiplegt_dyn/Stata"
use test_sample.dta, replace 
...
local 1 countk 
local 2 unique_small_grid_id 
local 3 monthyear 
local 4 downup_ac
local effects 5
local placebo 4 
local cluster "ac_uq_id"
local continuous = 0
local ci_level 95
local controls `CTRL_BASE' `ac_dummies'
...
local reg_failed_XX = 1
if "`trends_nonparam'" == "" {
capture reg prod_het_`i'_XX `predict_het_good' F_g_XX#d_sq_group_XX#S_g_XX [aw=N_gt_XX] if F_g_XX-1+`i'<=T_g_XX, level(`ci_level') vce(`ses')
local reg_failed_XX = _rc
}
if "`trends_nonparam'" != "" {
gegen trends_nonparam_temp_XX=group(`trends_nonparam')
capture reg prod_het_`i'_XX `predict_het_good' F_g_XX#d_sq_group_XX#S_g_XX#trends_nonparam_temp_XX [aw=N_gt_XX] if F_g_XX-1+`i'<=T_g_XX, level(`ci_level') vce(`ses')
local reg_failed_XX = _rc
capture drop trends_nonparam_temp_XX
}

// Retrieve the results from the regression only if it succeeded
if `reg_failed_XX' == 0 {
...
// Run regression of interest
local reg_pl_failed_XX = 1
if "`trends_nonparam'" == "" {
capture reg prod_het_pl_`i'_XX `predict_het_good' F_g_XX#d_sq_group_XX#S_g_XX [aw=N_gt_XX] if F_g_XX-1+`i'<=T_g_XX, level(`ci_level') vce(`ses') // leave the F_g_XX-1+`i'<=T_g_XX condition as it is to ensure that we only use treated groups or is this already taken into account by the S_g_XX?
local reg_pl_failed_XX = _rc
}
if "`trends_nonparam'" != "" {
gegen trends_nonparam_temp_XX=group(`trends_nonparam')
capture reg prod_het_pl_`i'_XX `predict_het_good' F_g_XX#d_sq_group_XX#S_g_XX#trends_nonparam_temp_XX [aw=N_gt_XX] if F_g_XX-1+`i'<=T_g_XX, level(`ci_level') vce(`ses')
local reg_pl_failed_XX = _rc
capture drop trends_nonparam_temp_XX
}

// Retrieve the results from the regression only if it succeeded
if `reg_pl_failed_XX' == 0 {
forvalues j=1/`:word count `predict_het_good''{
