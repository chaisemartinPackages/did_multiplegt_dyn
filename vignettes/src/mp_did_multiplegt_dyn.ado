...
version 12.0
syntax varlist(min=4 max=4 numeric) [if] [in] [, add(integer 1) effects(integer 1) placebo(integer 1) switchers(string) only_never_switchers controls(varlist numeric) trends_nonparam(varlist numeric) weight(varlist numeric max=1) dont_drop_larger_lower NORMALIZED cluster(varlist numeric max=1) same_switchers same_switchers_pl effects_equal drop_if_d_miss_before_first_switch trends_lin CONTinuous(integer 0) less_conservative_se bootstrap(string)]

    di ""
    di as text "Step 1: Initial run to retrieve dynamic effects."
    di as text "Errors related to the number of placebos can be ignored."
    qui {
        qui did_multiplegt_dyn `varlist', effects(`effects') placebo(`placebo') `switchers' `only_never_switchers' `effects_equal' controls(`controls') trends_nonparam(`trends_nonparam') weight(`weight') `dont_drop_larger_lower' `normalized' cluster(`cluster') `same_switchers' `same_switchers_pl' `drop_if_d_miss_before_first_switch' `trends_lin' continuous(`continuous') `less_conservative_se' bootstrap(`bootstrap') graph_off

        mat b_dyn = e(b)[1, 1..l_XX]
        mat b_ATE = e(b)[1, l_XX + l_placebo_XX + 1]
        mat V_dyn = e(V)[1..l_XX, 1..l_XX]
        mat V_ATE = e(V)[1+l_XX+l_placebo_XX, 1+l_XX+l_placebo_XX]
        scalar l_XX_og = l_XX
        scalar l_placebo_XX_og = l_placebo_XX
    }
    ...
    di ""
    di as text "Step 2: Running again with `add' null period(s) " _continue
    qui {
        add_periods `varlist', add(`add')
        if `effects' < `placebo' {
    ...
    di "=> `=l_placebo_XX - l_placebo_XX_og' more placebo(s)"  _newline
